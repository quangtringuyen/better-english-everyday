#!/usr/bin/env python3
"""
Extract Upper Intermediate Playlist and Auto-Rename
...
PLAYLIST_URL = "https://www.youtube.com/playlist?list=PL6vHaAQyQlk9XWaNN0HcA5-QnkvrSJi9w"
OUTPUT_DIR = Path("youtube_descriptions/Upper_Intermediate")
VIDEO_TITLES = {}
...
async def extract_video_description(page, video_url, video_number):
    ...
    try:
        ...
        try:
            expand_button = await page.wait_for_selector("tp-yt-paper-button#expand, button#expand", timeout=5000)
            if expand_button:
...
async def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════════════════════════╗
║           UPPER INTERMEDIATE PLAYLIST EXTRACTOR                              ║
║           Folder: Upper_Intermediate                                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """)
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if successful > 0:
                rename_files()
                print("\n🎉 COMPLETE!")
                print(f"✅ Extracted {successful} Upper Intermediate episodes")
